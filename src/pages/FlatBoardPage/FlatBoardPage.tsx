...
export default function FlatBoardPage({ boardKey }: FlatBoardPageProps) {
  ...
  const { getAddedItems, isDeleted, addItem, deleteItem, setField } = useOverrides();
  ...
  return (
    <>
      ...
      <AddTopicForm
        ...
        buttonLabel={`+ Добавить тему (${board.title})`}
        formTitle={`Новая тема: ${board.title}`}
        onAdd={(newTopic) => {
          addItem(boardTopicsGroupKey(boardKey), newTopic);
        }}
      />
    </>
  );
