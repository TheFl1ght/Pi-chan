...
export default function HigherTopicPage() {
  ...
  return (
    <>
      ...
      <TopicIntro title={topic.t} formula={<MathFormula mode="block">{topic.f}</MathFormula>} accent={board.accent} />
      {isDirectTabs ? (
        <TabsPanel
          theory={entry.theory}
        ...
        <>
          <SubtopicList
            ...
            items={subtopicsList.map((s, i) => ({
              title: s.h,
              formula: s.f ? <MathFormula mode="block">{s.f}</MathFormula> : undefined,
